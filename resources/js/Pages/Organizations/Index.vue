<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <Link class="btn-main" href="/organizations/create">
        <span>Create</span>
      ...
      <table class="w-full whitespace-nowrap">
        ...
        <tbody>
          <tr v-for="organization in organizations.data" :key="organization.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            <td class="border-t">
              <Link class="flex items-center px-6 py-4 focus:text-main-500" :href="`/organizations/${organization.id}`">
                {{ organization.name }}
                <icon v-if="organization.deleted_at" name="trash" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400" />
              </Link>
            </td>
            <td class="border-t">
              <Link class="flex items-center px-6 py-4" :href="`/organizations/${organization.id}`" tabindex="-1">
                {{ organization.city }}
              </Link>
            </td>
            <td class="border-t">
            ...
            <td class="w-px border-t">
              <Link class="flex items-center px-4" :href="`/organizations/${organization.id}`" tabindex="-1">
